...
## 费用：
- 开源版： 免费(请尊重原创，遵守Apache协议)  （代码地址：https://gitee.com/zuihou111 或者 https://github.com/zuihou )
- 个人版：(~~原价: 1500元~~) 现优惠价: 1299元/终身
- 企业版：(~~原价: 2000元~~) 现优惠价: 1899元/终身
...
| 功能 | 开源版 | 个人版 | 企业版 | 进度 | 描述 |
|---|---|---|---|---|---|
| 赠送看云上的开发文档 | 无 | 有 | 有 | 完成 | 先购买文档后购买个人版、企业版软件的用户不支持补差价 |
| 全部源码 | 有 | 有 | 有 | 完成 | 提供本项目相关的所有前、后端、公共模块、代码生成器源码, 没有闭源的jar (其三方jar不算) |
| 个人使用 | 有 | 有 | 有 | 完成 | 开源版:无需授权任意使用<br />个人版: 只允许个人学习、毕设使用，禁止商用。违反必究！！<br/> |
| 商业使用 | 有 | 无 | 有 | 完成 | 开源版:无需授权任意使用<br />企业版:允许个人和企业商业任意使用。<br/> |
| 允许将二开之后的源码提供给第三方 | 有 | 无 | 有 | 完成 | 开源版:只需要遵循Apache协议任意使用<br />个人版:源码禁止直接或者间接提供给第三方。<br/>企业版：源码可以二次开发或修改后可以提供给客户。<br/> 禁止用任何版本二次开发出同类产品售卖。 |
| 转卖、售卖、赠送源码、视频、文档给第三方 | 无 | 无 | 无 | 完成 | 违反必究！！！ |
| 基于本产品包装成同种类别的衍生产品进行源码、文档售卖盈利 | 无 | 无 | 无 | 完成 | 可以作为公司商业项目开发、交付使用，但严禁修改成类似SaaS快速开发平台、开发框架进行盈利 |
| 专属优先答疑会员群 | 无 | 无 | 有 | 完成 | 开源版、个人版通过源码、文档、视频、开源QQ群学习 |
| 是否提供发票 | 无 | 无 | 有 | 完成 |  |
...
| 功能 | 开源版 | 个人版 | 企业版 | 进度 | 描述 |
|---|---|---|---|---|---|
| redis、mq等中间件可插拔 | 有 | 有 | 有 | 完成 | 通过配置，一键开启关闭 |
| RBAC、其他常用功能 | 有 | 有 | 有 | 完成 |  |
| 极简的Oauth认证鉴权方案 | 有 | 有 | 有 | 完成 | 参考SpringSecurity、Shiro实现了一套极简的认证方案 |
| 多表、多服务关联查询数据注入 | 有 | 有 | 有 | 完成 | 基于注解实现数据注入 |
| 前后端表单统一验证 | 有 | 有 | 有 | 完成 | 前端组件尚未开发 |
| 统一缓存操作类 | 有 | 有 | 有 | 完成 | 统一key、统一接口、缓存空值 |
| 深度定制的前后端代码生成器 | 有 | 有 | 有 | 完成 | 基于mybatis-plus-generator实现 |
| 用户登录密码过期配置 | 有 | 有 | 有 |  完成 |  |
| 基于登录日志，做一些用户分析（用户地图等） | 有 | 有 | 有 | 未开发 |  |
| 第三方登录 | 有 | 有 | 有 | 未开发 | 支付宝、微信、QQ、gitee等登录 |
| 分布式事务 | 有 | 有 | 有 | 完成 | 数据源模式完美兼容分布式事务 |
| 表结构增强升级 | 无 | 有 | 有 | 完成 | 每个版本的表结构调整后，<br />会生成一个增量更新的脚本，<br />用于老用户升级 |
| 集成XXL-JOB分布式定时调度 | 无 | 无 | 有 | 完成 |  |
